 ...
 */
	public function startup() {
		...
		if (!empty($this->params['noninteractive'])) {
 ...
 */
	protected function _create($Schema, $table = null) {
		...
		if (!$this->isInteractive || 'y' == $this->in(__d('cake_console', 'Are you sure you want to drop the table(s)?'), array('y', 'n'), 'n')) {
		...
		if (!$this->isInteractive || 'y' == $this->in(__d('cake_console', 'Are you sure you want to create the table(s)?'), array('y', 'n'), 'y')) {
 ...
 */
	protected function _update(&$Schema, $table = null) {
		...
		if (!$this->isInteractive || 'y' == $this->in(__d('cake_console', 'Are you sure you want to alter the tables?'), array('y', 'n'), 'n')) {
			$this->out();
 ...
 */
	public function getOptionParser() {
		...
		$noninteractive = array(
			'short' => 'i',
			'help' => __d('cake_console', 'Run non-interactively (all questions will be answered "yes")'),
			'boolean' => true
		);
		$write = array(
		...
		$parser->description(
			...
			'parser' => array(
				'options' => compact('plugin', 'path', 'file', 'name', 'connection', 'dry', 'snapshot','noninteractive'),
				'args' => array(
					'name' => array(
